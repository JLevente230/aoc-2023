...
let input = '............................................411.....................363..134.........463.775..........................506...................\n' +
    ...
    '.........@........*.......8...31*...........+.....577*.........*....*...399....*.=....@......./...................................59........\n' +
    '.......740.......781..........................................105.353........791.579...........900.463..............909.....................';//fs.readFileSync('3/input.txt', 'utf-8');
...
function outOfBounds(number: number) {
    return number < 0 || number > rowNumber;
}

function checkIfPart(potentialPartIndices: number[], inputArray: string[], potentialPart: string): number {

    let isPart = false;

    potentialPartIndices.forEach(function (index) {

        for (let i = -1; i <= 1; i++) {
            let firstIndex = index - rowLength;
            let secondIndex = index;
            let thirdIndex = index + rowLength;

            if (!outOfBounds(firstIndex + i) && inputArray[firstIndex + i] !== '.' && !'0123456789'.includes(inputArray[firstIndex + i])) {
                //console.log(inputArray[firstIndex + i]);
                isPart = true;
                break;
            }
            if (!outOfBounds(secondIndex + i) && i !== 0 && inputArray[secondIndex + i] !== '.' && !'0123456789'.includes(inputArray[secondIndex + i])) {
                //console.log(inputArray[secondIndex + i]);
                isPart = true;
                break;
            }
            if (!outOfBounds(thirdIndex + i) && inputArray[thirdIndex + i] !== '.' && !'0123456789'.includes(inputArray[thirdIndex + i])) {
                //console.log(inputArray[thirdIndex + i]);
                isPart = true;
                break;
            }
        }
...
function checkExactPart(index: number, inputArray: string[]) {
    let indices = [];
    indices.push(inputArray[index]);
    let currentIndex = index;
    while ('0123456789'.includes(inputArray[currentIndex])) {
        if ('0123456789'.includes(inputArray[currentIndex + 1])) {
            indices.push(inputArray[currentIndex + 1]);
        }
        currentIndex++;
    }
    currentIndex = index;
    while ('0123456789'.includes(inputArray[currentIndex])) {
        if ('0123456789'.includes(inputArray[currentIndex - 1])) {
            indices.unshift(inputArray[currentIndex - 1]);
        }
        currentIndex--;
    }

    let result = '';
    indices.forEach(function (number) {
        result += number;
    });
...
function checkGear(index: number, inputArray: string[]) {

    let potentialParts:number[] = [];
    ...
    for (let i = -1; i <= 1; i++) {
        ...
        if (!outOfBounds(firstIndex + i) && '0123456789'.includes(inputArray[firstIndex + i])) {
            let maybe = checkExactPart(firstIndex + i, inputArray);
            if (!potentialParts.includes(maybe)) {
                potentialParts.push(maybe);
            }
        }
        if (!outOfBounds(secondIndex + i) && i !== 0 && '0123456789'.includes(inputArray[secondIndex + i])) {
            let maybe = checkExactPart(secondIndex + i, inputArray);
            if (!potentialParts.includes(maybe)) {
                potentialParts.push(maybe);
            }
        }
        if (!outOfBounds(thirdIndex + i) && '0123456789'.includes(inputArray[thirdIndex + i])) {
            let maybe = checkExactPart(thirdIndex + i, inputArray);
            if (!potentialParts.includes(maybe)) {
                potentialParts.push(maybe);
            }
        }
    ...
    if (potentialParts.length === 2) {
        return potentialParts[0] * potentialParts[1];
    }
...
for (let i = 0; i < inputArray.length; i++) {
    ...
    let potentialPartIndices: number[] = [];
    let potentialPart = '';
    if ('0123456789'.includes(inputArray[i])) {
        potentialPartIndices.push(i);
        potentialPart += inputArray[i];
        i++;
        while ('0123456789'.includes(inputArray[i])) {
            potentialPartIndices.push(i);
            potentialPart += inputArray[i];
            i++;
        }

        result += checkIfPart(potentialPartIndices, inputArray, potentialPart);
    }

    if (inputArray[i] === '*') {
        result2 += checkGear(i, inputArray);
    }
}
